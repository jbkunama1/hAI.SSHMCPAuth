...
Diese Datei beschreibt, wie Agenten den MCP-Server **hAI.SSHMCPAuth** nutzen sollen, um sicher per MCP über einen API-Key auf SSH-Ziele im LAN zuzugreifen.

## 1. Überblick

- **Projektname:** hAI.SSHMCPAuth  
- **Zweck:** Auth-layered MCP SSH-Gateway für sicheren Agentenzugang in dein LAN.  
- **Architektur:** Node.js API-Key-Proxy vor einem bestehenden `ssh-mcp`-Server.  
- **Einsatzszenario:** Zugriff von LLM-Agenten auf SSH-fähige Systeme im LAN über MCP, abgesichert durch einen API-Key.

## 2. Sicherheits-Hinweise

- Dieses Projekt ist ein Zugangsgateway zu SSH-fähigen Systemen.  
- Nutze es nur in einem **vertrauenswürdigen Netzwerk** oder **hinter einem VPN/Tunnel mit HTTPS**.  
- Verwende **dedizierte, minimal berechtigte SSH-Accounts** und **bevorzuge SSH Keys statt Passwörtern**.  
...
- **Docker** installiert.  
- **Docker Compose** installiert, um den bereitgestellten Docker-Compose-Stack auszuführen.  
- Zugriff auf eine gültige **SSH-Infrastruktur** (Ziele im LAN mit SSH-Servern).  
...
- Der MCP-Server verwendet einen **API-Key** zur Authentifizierung von Agenten.  
- Der API-Key wird über die Umgebungsvariable **`SSHMCP_API_KEY`** konfiguriert.  

### Richtlinien für Agenten

- Gehe davon aus, dass **ohne gültigen API-Key kein Zugriff** auf den Gateway möglich ist.  
...
- Ein **Auth-Layer** vor einem bestehenden `ssh-mcp`-Server: Der Auth-Layer nimmt MCP-Anfragen entgegen und reicht sie nur dann an `ssh-mcp` weiter, wenn der API-Key gültig ist.  
- Ein **Gateway in das LAN**: Der Zugriff erfolgt nicht direkt über öffentliche IPs, sondern über das interne Netzwerk.  
...
- Es existiert ein veröffentlichtes Container-Image für dieses Projekt (z. B. via GHCR).  
- Konkrete Image-Namen, Compose-Dateien und Startbefehle sind in der Projekt-README und in der Deployment-Umgebung dokumentiert.  
...
- hAI.SSHMCPAuth ist ein **API-Key-geschützter MCP-Gateway** vor einem `ssh-mcp`-Server für sicheren SSH-Zugriff im LAN.  
- Du arbeitest immer **authentifiziert** (API-Key) und in einem **vertrauenswürdigen Netzwerk** (bzw. hinter VPN/Tunnel mit HTTPS).  
- Du nutzt **least privilege**: so wenig Aktionen wie nötig, so sicher wie möglich.
